...
2. **任务列表操作**
    ...
    - **移动：** 鼠标拖拽移动更改排序，内置中线判定引擎（左半边前插，右半边后插）。多选后可批量平移位置。
      - **Alt 键克隆：** 在拖拽任何卡片的过程中，**按住 `Alt` 键**，即可触发物理克隆模式（鼠标指针会变为带有 `+` 号的复制样式）。松开鼠标后，系统会在目标位置生成一份完全一致的**无损深拷贝**（完美保留所有绑定的参数配置及完整的历史生成记录），且原件保持不动。克隆完成后，选中焦点会自动转移到新生成的卡片上。
...
2. **模块操作**
    ...
    - **拖拽与无损克隆 (Alt键)：** 结合上述的“平行同步”或“跨卡片聚合”拖拽，只要在拖拽过程中**按住 `Alt` 键**，即可将单纯的移动升级为**无损克隆**！松开鼠标后，系统会在目标位置生成完全一致的深拷贝（连同完整的历史出图/视频记录一起原样复制），且原件保持不动。克隆完成后，选中焦点会自动转移到新生成的模块上。
3. **工具栏 (模块级)**
...
- **文本组件：**
    ...
    - 提供 **语法高亮**、**预览Markdown**、**复制全文** 三个能力；正文使用无衬线字体显示，预览区会根据文本高度自适应，并在长文本时启用细滚动条。
- **通用文件组件：**
